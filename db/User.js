...
import { log, greenCheck } from '../util/util.js'
...
userSchema.statics.newUser = (userObj) => {
          ...
          newUser
            ...
            .then(() => {
              User.find({
                ...
                log(res, true)
                log(
                  `${greenCheck}new user '${res[0].handle}' successfully made`,
                  true
                )
                resolve(newUser)
              })
...
userSchema.virtual('isAdmin').get(() => {
  ...
  if (
    this.connections.discord &&
    this.connections.discord === '153983024411836416'
  ) {
    isAdmin = true
  }
  // find admin scope
...
/* Static Methods */
userSchema.statics.listUsers = () => {
  return new Promise((resolve, reject) => {
    User.find()
      .then((res) => {
        let txt = `__**${res.length} user${
          res.length == 1 ? '' : 's'
        ...
        res.forEach((user) => {
          txt +=
            ...
            (user.handle
              ? user.handle
              : user.connections.discord.discord_handle) +
            '\n'
          index++
        })
        resolve(txt)
...
userSchema.statics.fromDiscordId = async (discordId) => {
  return (await User.find({ 'connections.discord.discordId': discordId }))[0]
}

userSchema.statics.fromUserId = (mongooseId) => {
  return User.findById(mongooseId)
}

/* Methods */

userSchema.methods.setMain = (character) => {
  ...
  log(
    `User.setmain: setting ${character.fullName} as default for ${User.handle}`,
    true
  )
  if (charUId == userId) {
...
userSchema.methods.grantRole = (role) => {
  if (typeof role != 'string')
    throw 'User.grantRole Error: role must be a string, not ' + typeof role
  if (!ROLE_LIST.includes(role))
    throw `User.grantRole Error: '${role}' is not a valid role`
  return new Promise((resolve, reject) => {
...
userSchema.methods.removeRole = (role) => {
  if (typeof role != 'string')
    throw 'User.grantRole Error: role must be a string, not ' + typeof role
  if (!ROLE_LIST.includes(role))
    throw `User.grantRole Error: '${role}' is not a valid role`
...
/* Private */
const _loginUserSync = (user) => {
  user.last_seen = Date.now()
  user.save()
}

const checkNewUser = async (userObj) => {
  log('User.checkNewUser: checking user...')
  log(userObj)

  const handle = userObj.handle
  if (!handle) throw 'user must have a name'
  ...
  } else if (handle >= MAX_HANDLE_LENGTH) {
...
const initializeFields = (userObj) => {
  log('User.initializeFields userObj:')
  log(userObj)
  if (typeof userObj.handle == 'undefined')
    throw 'User.initializeFields tried to initialize user without handle'
  let newUserObj = {
    isDeleted: false,
    ...
    defaultCharacter:
      typeof userObj.defaultCharacter == 'undefined'
        ? undefined
        : userObj.defaultCharacter,
    name:
      typeof userObj.name == 'undefined'
        ? { first: 'Mystery', last: 'Sister' }
        : userObj.namefirstlast,
    current_system:
      typeof userObj.current_system == 'undefined'
        ? 'of a down'
