...
characterSchema.statics.newCharacter = (charObj) => {
...
characterSchema.virtual('charString').get(() => {
  return `${this.firstName} the level ${this.level} ${this.charRace} ${this.charClass}`
})

characterSchema.virtual('fullName').get(() => {
...
characterSchema.virtual('level').get(() => {
  return levelFromExperience(this.experience)
})

characterSchema.virtual('remainingExperience').get(() => {
  return remainingExperience(this.experience)
})

characterSchema.virtual('moneyToString').get(() => {
  let moneyString = ''
...
characterSchema.methods.addExperience = async (amount) => {
  if (typeof amount != 'number')
    throw `Character.addExperience ERROR: ${amount} is not a number`
  this.experience += amount
  return this.save()
}

characterSchema.methods.addResonite = () => {
  log('the addResonite function is unfinished', true)
  // get ( this.id)
  // .addresonite
}

characterSchema.methods.removeResonite = () => {
...
/* Statics */
characterSchema.statics.listCharacters = () => {
  return new Promise((resolve, reject) => {
    Character.find()
      .then((res) => {
        let txt = `__**${res.length} character${
          res.length == 1 ? '' : 's'
        } currently initialized:**__\n`
...
characterSchema.statics.fromUserId = (userMongooseId) => {
  log('userMongooseId')
  log(userMongooseId)
...
characterSchema.statics.fromCharacterId = (characterMongooseId) => {
...
const checkNewCharacter = async (charObj) => {
  log('checking character...', true)
  if (!charObj.firstName) throw 'character must have a name'
  let fullName =
    charObj.firstName + (charObj.lastName ? ' ' + charObj.lastName : '')

  // check to make sure that the character has a unique name
  let charOK = true

  if (charObj.firstName.length + charObj.lastName.length < 3) {
    throw `the name "${fullName}" is too short. Character names must be at least 3 characters.`
  } else if (charObj.firstName.length + charObj.lastName.length >= 32) {
    throw `the name "${fullName}" is too long. Character names cant be over 32 characters.`
  } else {
  ...
  let namePromise = Character.find({
    ...
    .then((res) => {
      log(
        `Character name search for ${charObj.firstName} ${charObj.lastName}:`,
        true
      )
      log(res, true)
...
const levelFromExperience = (experience) => {
  if (typeof experience != 'number')
    throw 'cannot find level from a non-integer amount of experience'
  else {
    return characterOptions.levelThresholds.findIndex(
...
const remainingExperience = (experience) => {
  if (typeof experience != 'number')
    ...
    let nextLvl = characterOptions.levelThresholds.findIndex(
      (element) => element > experience
    )
...
const initializeFields = (charObj) => {
  log('initializeFields(charObj):')
  log(charObj)
  let newCharObj = {
    firstName:
      typeof charObj.firstName == 'undefined' ? 'DEFAULT' : charObj.firstName,
    lastName:
      typeof charObj.lastName == 'undefined' ? 'DEFAULT' : charObj.lastName,
    nickName:
      typeof charObj.nickName == 'undefined' ? 'DEFAULT' : charObj.nickName,
    nameShort:
      typeof charObj.nameShort == 'undefined' ? 'DEFAULT' : charObj.nameShort,
    experience:
      typeof charObj.experience == 'undefined' ? 0 : charObj.experience,
    charRace:
      typeof charObj.charRace == 'undefined' ? 'Waxoid' : charObj.charRace,
    charGender:
      typeof charObj.charGender == 'undefined' ? 'agender' : charObj.charGender,
    charClass:
      typeof charObj.charClass == 'undefined' ? 'Peasant' : charObj.charClass,
    charSubclass:
      typeof charObj.charSubclass == 'undefined'
        ? 'Filthy'
        : charObj.charSubclass,
    currentActivity:
      typeof charObj.currentActivity == 'undefined'
        ? 'Waiting'
        : charObj.currentActivity,
    moneyCp: typeof charObj.moneyCp == 'undefined' ? 1000 : charObj.moneyCp,
    location:
      typeof charObj.location == 'undefined'
        ? 'Foxbarrow Farms'
        : charObj.location,
    inventory: typeof charObj.inventory == 'undefined' ? {} : charObj.inventory,
    hitDieCurrent:
      typeof charObj.hitDieCurrent == 'undefined' ? 1 : charObj.hitDieCurrent,
    ...
    reputations:
      typeof charObj.reputations == 'undefined' ? {} : charObj.reputations,
    conditions:
