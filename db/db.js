...
function editEntry(coll, filter, update, options) {
  return new Promise((resolve, reject) => {
    getElementIn(filter, coll)
        ...
        if (res.length == 1) {
          ...
          db.collection(coll).updateOne(
            ...
            (err, result) => {
              ...
              if ((result.modifiedCount = 1)) {
                log(
                  `successfully modified ${result.modifiedCount} entries!`,
                  true
                )
                resolve(result)
              }
      ...
      .catch((err) => {
        throw (
          'expected to find 1 entries for given filter but found ' + res.length
        )
      })
  })
}
    ...
    db.collection('monsters').updateOne(
      ...
      (err, result) => {
        ...
        if (result.modifiedCount > 0)
          log(
            'Success! ' + result.modifiedCount + ' etnries were modified',
            true
          )
        else log('Everything went through but nothing was modified.', true)
        resolve(result)
      }
    )
  })
  ...
  return new Promise((resolve, reject) => {
    db.collection(coll, { strict: true }, (err, col) => {
      ...
      col.deleteOne(filter, (err, result) => {
        ...
        else if (result.deletedCount == 0) resolve('No entry was deleted')
      })
    })
  })
}
...
function getCollectionInformation(targetCollection) {
  log(
    'Attempting to get information on the collection: ' + targetCollection,
    true
  )
  return new Promise((resolve, reject) => {
...
function newColl(collname) {
  ...
  return new Promise((resolve, reject) => {
    db.createCollection(collname).then(
      (result) => {
        resolve(result)
      },
      (reason) => {
