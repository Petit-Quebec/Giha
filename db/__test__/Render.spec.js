...
describe('Render test', () => {
    ...
    it('should be able to render all locations together', async () => {
        ...
        expect(imgData.slice(20,24)).toStrictEqual(Buffer.from([Hbyte4,Hbyte3,Hbyte2,Hbyte1]))
        expect(imgData.slice(16,20)).toStrictEqual(Buffer.from([Wbyte4,Wbyte3,Wbyte2,Wbyte1]))
    })
    
    it('should throw when called on something other than a map', async () => {
        await expect(render('not a map')).rejects.toEqual('Map should be of type InstanceMap, not string')
        await expect(render(50)).rejects.toEqual('Map should be of type InstanceMap, not number')
    })
})
