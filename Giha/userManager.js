...
function initializeUser(newUser) {
	if (!newUser.hasOwnProperty('discordId'))
	...
	return new Promise((resolve, reject) => {
		...
		getUser(newUser.discordId)
			...
			.then((user) => {
				...
				if (user)
					reject(
						`LOOMERROR: user already initialized: ${user.discordHandle}`
					)
				else {
					...
					let newUserDoc = new User({
						...
						connections: {
							discord: {
								discord_handle:
									'discordHandle' in newUser
										? newUser.discordHandle
										: 'null',
