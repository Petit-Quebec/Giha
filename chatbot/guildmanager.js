...
const parseGuilds = (connectedGuilds, initializedGuilds) => {
  if (initializedGuilds.length == 0)
    throw 'parseGuilds was called with no active guilds'
  let activeGuilds = []
  let inactiveGuilds = []
  let passiveGuilds = []
  ...
  initializedGuilds.forEach((initializedGuild) => {
    let found = false
    for (
      let connectedGuildIndex = 0;
      connectedGuildIndex < connectedGuilds.length;
      connectedGuildIndex++
    ) {
      const connectedGuild = connectedGuilds[connectedGuildIndex]
      if (initializedGuild.id == connectedGuild.id) {
        log(
          'Guild "' +
            initializedGuild.name +
            '" is active with type ' +
            initializedGuild.type
        )
  ...
  connectedGuilds.forEach((connectedGuild) => {
    let found = false
    for (
      let initializedGuildIndex = 0;
      initializedGuildIndex < initializedGuilds.length;
      initializedGuildIndex++
    ) {
      const initializedGuild = initializedGuilds[initializedGuildIndex]
      if (initializedGuild.id == connectedGuild.id) {
        found = true
        break
      }
...
// const initializeGuild = (guild, isAdmin) => {
//   db.getElementIn({ id: guild.id }, 'guilds').then((res) => {
//     if (res.length > 0) {
//       log(
//         '\x1b[31mWARNING:\x1b[0m Guild ' +
//           guild.name +
